...
<nav class="navbar navbar-expand-lg bg-light ">
 ...
 <?php
 $allProducts = $db->query('SELECT * FROM products ORDER BY id DESC');
 
 if(isset($_GET['s']) AND !empty($_GET['s'])){
  $recherche = htmlspecialchars($_GET['s']);
  $allProducts = $db->query('SELECT * FROM products WHERE name LIKE "%'.$recherche.'%" ORDER BY id DESC');
 }
 ...
 ?>    
      ...
      <form class="d-flex" role="search">
        <input class="form-control me-2" type="search" name="s" placeholder="Rechercher un produit" aria-label="Search">
...
<?php
if($allProducts->rowCount() > 0 && isset ($_GET["envoyer"])){
  while($all = $allProducts->fetch()){
   
   ?>
<div class="flex card" >

  <div class="card-body">
    <h1 class="card-title"><?=$all['name']?></h1>
    <a href="achat.php?id=<?=$all['id'] ?>" class="btn btn-primary stretched-link"><?=$all['price']?> €</a>
  </div>
</div>
<?php
  }
